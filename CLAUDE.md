...
- `manifest.json` — Chrome MV3 manifest. Includes `browser_specific_settings.gecko` with `data_collection_permissions: { required: ["none"] }` for Firefox/AMO. Chrome ignores unrecognized top-level keys, so one manifest works for both browsers. **Content scripts are no longer declared statically** — `background.js` registers them dynamically via `chrome.scripting.registerContentScripts` so the `excludeMatches` list can be rebuilt from `chrome.storage.local.disabledDomains` at runtime. The list always includes `*://challenges.cloudflare.com/*` (Turnstile iframes — they probe ~130 fingerprinting APIs and detect our wrappers, sending the challenge into a refresh loop) plus user-disabled domains (toggled via the power button in the popup header). Requires `permissions: ["scripting"]` and `host_permissions: ["<all_urls>"]`.
...
- **Don't remove `*://challenges.cloudflare.com/*` from `ALWAYS_EXCLUDE` in `background.js`'s `registerDynamicScripts()`.** Turnstile iframes run a full anti-tamper fingerprint suite that detects our wrappers; injecting there sends Turnstile into a verification loop and prevents the user from completing the challenge. Verified on dl.acm.org. The exclude pattern matches per-frame URL so it covers Turnstile iframes embedded on any parent site.
- **Per-site disable rebuilds `excludeMatches`, not in-page muting.** When the user clicks the power button in the popup, `chrome.storage.local.disabledDomains` is updated and `background.js`'s storage listener calls `chrome.scripting.unregisterContentScripts` + `registerContentScripts` to refresh the registration. The popup then `chrome.tabs.reload`s the active tab so the change takes effect on the current page. The mute system is separate — it suppresses recording but leaves hooks installed (which is what trips Turnstile); only `disabledDomains` truly skips injection.
